...
Connection.prototype._writeControlFrame = function _writeControlFrame(frame) {
  ...
  } else if (frame.type == 'BLOCKED') { // Some server implementations are using this deprecated frame type. Removed in draft-12, be permissive.
    this._log.error({ frame: frame }, 'Invalid BLOCKED frame');
  ...
  for (var priority in this._streamPriorities) {
    ...
    while (bucket.length > 0) {
      for (var index = 0; index < bucket.length; index++) {
        ...
        var frame = stream.upstream.read(4096);//(this._window > 0) ? this._window : -1);

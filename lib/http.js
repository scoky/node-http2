...
exports.convertHeadersToH2 = function convertHeadersToH2(headers) {
  var nhdrs = {}
  Object.keys(headers).forEach(function (key) {
    if (key == "host") nhdrs[':authority'] = headers[key]
    else if (key == "strict-transport-security") { }
    else if (deprecatedHeaders.indexOf(key) == -1) nhdrs[key] = headers[key]
  })
  return nhdrs
}

exports.convertHeadersFromH2 = function convertHeadersFromH2(headers) {
  var nhdrs = {}
  Object.keys(headers).forEach(function (key) {
    if (key == ":authority") { nhdrs['host'] = headers[key] }
    else if (key == "strict-transport-security") { }
    else if (key.lastIndexOf(':', 0) !== 0) { nhdrs[key] = headers[key] }
  })
  return nhdrs
}

// When doing NPN/ALPN negotiation, HTTP/1.1 is used as fallback
var supportedProtocols = [protocol.VERSION, 'h2', 'h2-15']; //, 'http/1.1', 'http/1.0'];
//var supportedProtocols = ['http/2.0'];
...
function Server(options) {
  ...
  this._log.info({protocol : protocol}, "Desired protocol");
...
Server.prototype._fallback = function _fallback(socket) {
  ...
  // PREVENTING ANYTHING LESS THAN http/2 for testing purposes -Kyle
  //throw new Error('Fallback to < h2-14 not allowed.');
...
Agent.prototype.request = function request(options, callback) {
  ...
  // * HTTP/2 over plain TCP
    ...
    //throw new Error('HTTP2 over plain TCP is not allowed.');
  ...
  // * HTTP/2 over TLS negotiated using NPN or ALPN, or fallback to HTTPS1
    ...
    options.ALPNProtocols = supportedProtocols;
    options.NPNProtocols = supportedProtocols;
    // Do not use the proxy as the servername
    options.servername = options.servername || options.host; // Server Name Indication
    options.agent = this._httpsAgent;
    options.ciphers = options.ciphers || cipherSuites;
    var httpsRequest = https.request(options);

    httpsRequest.on('error', function(err) {
	self._log.info({ e: err }, 'Error on establishing connection');
    });

    httpsRequest.on('socket', function(socket) {
      // Alert that a new connection is established
      if (request.listeners('newConnection').length > 0 && socket) {
        request.emit('newConnection', { 
    ...
    var self = this;
    function negotiated() {
      if (request.listeners('certificateValid').length > 0) {
        request.emit('certificateValid', httpsRequest.socket.authorized);
      }

      var endpoint;
      var negotiatedProtocol = httpsRequest.socket.alpnProtocol || httpsRequest.socket.npnProtocol;

      // Report the negotiated protocol to higher levels
      if (request.listeners('protocolNegotiated').length > 0) {
        request.emit('protocolNegotiated', negotiatedProtocol);
      }

      if (negotiatedProtocol === protocol.VERSION || (negotiatedProtocol && negotiatedProtocol.indexOf('h2') === 0)) {
...
OutgoingRequest.prototype._start = function _start(stream, options) {
  ...
  headers[':scheme'] = headers[':scheme'] || options.protocol.slice(0, -1);
  headers[':method'] = headers[':method'] || options.method;
  headers[':authority'] = headers[':authority'] || options.host;
  headers[':path'] = headers[':path'] || options.path;

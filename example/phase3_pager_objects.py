...
class Fetch(object):
    def __init__(self, key, page, protocol, url, new_connection, push, size, fetch_time, prior, code, after_visit):
        self.key = key
        self.page = page
...
if __name__ == "__main__":
    ...
    for page,p in data[SPDY].iteritems():
        ...
        p['median'] = result
        try:
          printObjects(page, SPDY, result + additional)
    ...
    for page,p in data[H2].iteritems():
        ...
        if page in data[H1]:
            ...
            for r in replaced:
                result.remove(r)
        try:
          printObjects(page, H2, result + additional)
          #d = parseData(result + additional)
          #output(page, H2, d)
        except Exception as e:
          sys.stderr.write('Error on %s: %s\n' % (page, e))
